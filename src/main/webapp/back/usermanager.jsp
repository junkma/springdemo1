...
<div>
    ...
    <table border="1" cellpadding="0"cellspacing="0" >
        ...
        <c:forEach items="${requestScope.users.lists}" var="user">
            <tr>
                ...
                <td style="text-align: center"><a href="selectuser?id=${user.id}">修改</a>&nbsp;<a href="deluser?id=${user.id}" onclick="return confirm('你确定要删除吗？')">删除</a></td>
            </tr>
        </c:forEach>
    </table>
    ...
    <span style="text-align: center" >
          <a href="${pageContext.request.contextPath }/userManager?currentPage=1">[首页]</a>
<c:if test="${requestScope.users.currPage !=1}">
    <a href="${pageContext.request.contextPath }/userManager?currentPage=${requestScope.user.currPage-1}">[上一页]</a>
    <c:if test="${requestScope.users.currPage ==1}">
        <a href="${pageContext.request.contextPath }/userManager?currentPage=1">[上一页]</a>
    </c:if>
</c:if>
<c:if test="${requestScope.users.currPage !=requestScope.users.totalPage}">
    <a href="${pageContext.request.contextPath }/userManager?currentPage=${requestScope.users.currPage+1}">[下一页]</a>
    <c:if test="${requestScope.users.currPage==requestScope.users.totalPage}">
        <a href="${pageContext.request.contextPath }/userManager?currentPage=${requestScope.users.totalPage}">[下一页]</a>
    </c:if>
</c:if>
        <a href="${pageContext.request.contextPath }/userManager?currentPage=${requestScope.users.totalPage}">[尾页]</a>
