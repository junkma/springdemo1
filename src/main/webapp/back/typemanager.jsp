...
<%  %>
    ...
    <tr>
       ...
       <td style="text-align: center"><a href="selecttype?id=${type.id}">修改</a>&nbsp;<a href="deltype?id=${type.id}" onclick=" return confirm('是否要删除吗?')">删除</a></td>
    </tr>
        <c:forEach items="${type.childname}" var="child">
            <tr>
                <td style="text-align: center">${child.id}</td>
                <td style="padding-left: 60px">${child.name}</td>
                <td style="text-align: center"><a href="selecttype?id=${child.id}">修改</a>&nbsp;<a href="deltype?id=${child.id}" onclick=" return confirm('是否要删除吗?')">删除</a></td>
