...
<div>
 ...
 <c:forEach items="${requestScope.paper.lists}" var="comment">
   <tr>
       ...
       <td style="text-align: center"><a href="select?id=${comment.id}">回复</a>&nbsp;<a href="del?id=${comment.id}" onclick="return confirm('你确定要删除吗？')">删除</a></td>
   </tr>
 </c:forEach>
    </table>
...
<br/>
    <span style="text-align: center" >
          <a href="${pageContext.request.contextPath }/commentManager?currentPage=1">[首页]</a>
<c:if test="${requestScope.paper.currPage !=1}">
         <a href="${pageContext.request.contextPath }/commentManager?currentPage=${requestScope.paper.currPage-1}">[上一页]</a>
</c:if>
<c:if test="${requestScope.paper.currPage !=requestScope.paper.totalPage}">
          <a href="${pageContext.request.contextPath }/commentManager?currentPage=${requestScope.paper.currPage+1}">[下一页]</a>
</c:if>
        <a href="${pageContext.request.contextPath }/commentManager?currentPage=${requestScope.paper.totalPage}">[尾页]</a>
       <c:if test="${requestScope.paper.currPage ==1}">
           <a href="${pageContext.request.contextPath }/commentManager?currentPage=1">[上一页]</a>
       </c:if>
       <c:if test="${requestScope.paper.currPage==requestScope.paper.totalPage}">
           <a href="${pageContext.request.contextPath }/commentManager?currentPage=${requestScope.paper.totalPage}">[下一页]</a>
       </c:if>
       </span>
