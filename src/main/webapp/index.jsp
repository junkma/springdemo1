...
<div>
    <div  id="top">
           ...
           <span id="top1">
               ...
               <a href="off">注销</a>&nbsp;&nbsp;<a href="checkCar">购物车</a>&nbsp;&nbsp;<a href="login.jsp">登录</a>&nbsp;&nbsp;<a href="regist.jsp">注册</a>&nbsp;&nbsp;<a href="comment">留言</a>&nbsp;&nbsp;
             <c:if test="${!empty user}">
           ...
           <div id="left1">
              ...
              <div >
                  <ul>
                      ...
                      <li><a href="productType?id=${child.id}" target="right">${child.name}</a></li>
                      </c:forEach>
                  </ul>
              </div>
               </c:forEach>
           ...
           <div id="left2">
               ...
               <c:if test="${!empty browses}">
                   <c:forEach items="${browses}" var="browse">
                       <a href="productDetail?id=${browse.product.id}"><img src="front/static/images/${browse.product.filename}" width="50px" height="50px"/></a>
                       <a href="productDetail?id=${browse.product.id}">${browse.product.name}</a><br/>
                   </c:forEach>
               </c:if>
           </div>
       </div>
       <div id="center">
           <iframe scrolling="auto" rameborder="0" src="allProducts" name="right" width="100%" height="100%" ></iframe>
       </div>
       <div id="right">
          <ul>
              <li style="background: #e4e4e4;width: 240px;height: 30px;color: #000000">新闻动态</li>
              <marquee  behavior="alternate" scrollamount='1' scrolldelay='20' direction= 'up' width='250' id='akuma' height='200' onmouseover='akuma.stop()' onmouseout='akuma.start()'>
                   <c:if test="${!empty news}">
                       <c:forEach items="${news}" var="news">
                           <li><a href="newsDetail?id=${news.id}" target="right">${news.title}</a></li>
                       </c:forEach>
                   </c:if>
               </marquee>
           </ul>
