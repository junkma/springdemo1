            ...
            <li class="lx">
                <div id="luxian">
                    <ul>
                        ...
                        <li><a href="productDetail?id=${car.product.id}">${car.product.name}</a></li>
                    </ul>
                </div>
            </li>
            <li class="rs">￥${car.product.price}</li>
            <li class="dj">
                <a href="minCar?id=${car.id}&&pagenum=${pageInfo.pageNum}"><input type="button" value="-" class="btn btnMinus" /></a>
                <input type="text" class="txt" value="${car.number}" disabled="disabled" size="3"/>
                <a href="addCar?id=${car.id}&&pagenum=${pageInfo.pageNum}"><input type="button"  value="+" class="btn btnAdd" /></a>
            </li>
            <li class="cz"><a href="deleteCar?id=${car.id}" class="btnDel" onclick="return confirm('确定是否删除?')"><span style="color:#F60; text-decoration:underline;">删除</span></a></li>
        </ul>
    </div>
    </c:forEach>
    <div id="fanye">
        共${pageInfo.total}条记录&nbsp;&nbsp;&nbsp;&nbsp;${pageInfo.pageNum}/${pageInfo.pages}页<br/>
        <a href="checkCar?pagenum=1">首页</a> <a href="checkCar?pagenum=${pageInfo.pageNum-1}">上一页</a> <a href="checkCar?pagenum=${pageInfo.pageNum+1}">下一页</a> <a href="checkCar?pagenum=${pageInfo.pages}">尾页</a>
    </div>
    <div id="jine">应付金额：<span style="color:#F00;">${sumPrice}</span>元</div>

    <div id="js">
        <a href="submitOrder?cost=${sumPrice}"><img src="front/static/images/js.jpg" /></a>
    </div>
</div>
